...
class MainWindow(QMainWindow, WindowMixin):
   ...
   #     self.labelMethod = 'detect'
        ...
        self.textContainer.setLayout(textLayout)
        self.textdock = QDockWidget(u'Box Text', self)
        self.textdock.setObjectName(u'Text')
        self.textdock.setWidget(self.textContainer)
        
        #add a window to dispaly other information
        ...
        self.infodock = QDockWidget(u'Box Info', self)
        self.infodock.setObjectName(u'Text')
        self.infodock.setWidget(self.infoContainer)
        ...
        scroll = QScrollArea()
        #centerLayout = QFormLayout()
        #centerLayout.addRow(self.canvas)
        #centerLayout.addRow(self.textdock)
        #scroll.setLayout(centerLayout)
        centerLayout = QVBoxLayout()
        centerLayout.addWidget(self.textdock)
        centerLayout.addWidget(self.infodock)
        scroll.setLayout(centerLayout)
        ...
        open = action('&Open', self.openFile,
                'Ctrl+O', 'open', u'Open image or label file', enabled = False)

        opendir = action('&Open Dir', self.openDir,
                'Ctrl+u', 'open', u'Open Dir',enabled = False)

        changeSavedir = action('&Change save dir', self.changeSavedir,
                'Ctrl+r', 'open', u'Change default saved Annotation dir',enabled = False)
        
        enterProjectName = action('&Enter the project name', self.enterProjectName,
                'Ctrl+m', 'enter', u'Enter the exact project name')
        ...
        save = action('&Save', self.saveFile,
                'Ctrl+S', 'save', u'Save labels to filexxx', enabled=False)
        switchMode = action('&Switch\nMode', self.switchMode,
                'Ctrl+R','mode',u'switch mode or not ')
        ...
        hideAll = action('&Hide\nRectBox', partial(self.togglePolygons, False),
                'Ctrl+H', 'hide', u'Hide all Boxs', enabled=False)
        showAll = action('&Show\nRectBox', partial(self.togglePolygons, True),
                'Ctrl+A', 'hide', u'Show all Boxs',
                enabled=False)

        showMarkPro = action('&Show\nMark\nProgress', self.showMarkPro,
                'Ctrl+P', 'hide', u'Show Mark Progress')

        help = action('&Tutorial', self.tutorial, 'Ctrl+T', 'help',
                u'Show demos')
        verify = action('&Verify text', self.verify, 
                u'verify')
        right = action('&Set as right', self.right, 
                u'right')
        wrong = action('&Set as wrong', self.wrong, 
                'Ctrl+J','hide',u'wrong')
        ...
        addActions(self.menus.view, (
            ...
            hideAll, showAll, showMarkPro, None,
        ...
        self.actions.beginner = (
            ...
            changeSavedir, opendir, openNextImg, openPrevImg, switchMode , None, create, copy, delete, None,
   ...
   #     libs.canvas.labelMethod(self.labelMethod)
    ...
    def showText(self,text):
        #self.text.setText('hahahahahaha')
        rowlength = 50
        strlen = rowlength
        while strlen < len(text):
            text = text[:strlen]+'\n'+text[strlen:]
            strlen = strlen + rowlength
        self.textLabel.setText(text)
        ft =  QFont()
        ft.setPointSize(15)
        self.textLabel.setFont(ft)
        #self.textLabel.wordWrap = True
    ...
    def autoRight(self):
        if self.verifyImgDic.has_key(self.imagepath):
            return
        else:
            lso.veriResult('is_verified_right')
            print "You have set the status of the image as right!"

    def right(self):
        if self.verifyImgDic.has_key(self.imagepath):
            del self.verifyImgDic[self.imagepath]
        lso.veriResult('is_verified_right')
        print "You have set the status of the image as right!"


    def wrong(self):
        self.verifyImgDic[self.imagepath] = 'wrong'
        lso.veriResult('is_verified_wrong')
        print "You have set the status of the image as wrong!"
        yes, no = QMessageBox.Yes, QMessageBox.No
        msg = u'You have set the image as wrong'
        QMessageBox.warning(self, u'Information', msg)


    def show_results(self):
        rightNum = 0
        wrongNum = 0
        #for value in self.verifyImgDic.values():
        #    if value == 'right':
        #        rightNum += 1
        #    elif value == 'wrong':
        #        wrongNum += 1
        ...
        yes, no = QMessageBox.Yes, QMessageBox.No
        msg = u'Correct images: %s, wrong images: %s\ncorrect rate: %s\nNumber of veried images: %s\nveried data set range:%s / %s = %s '% resultList
        if yes == QMessageBox.warning(self, u'Show results', msg, yes):
            return
    ...
    def editLabel(self, item=None):
        ...
        if text is not None:
            item.setText(text)
            self.textDict[self.filename] = text
            self.setDirty()
    ...
    #def remLabel(self, item):
        ...
        self.openNextImg()
        self.openPrevImg()
        # Proceding elf.saveFile()

        #self.labelList.takeItem(self.labelList.row(item))
        # del self.shapesToItems[shape]
        # del self.itemsToShapes[item]
         
        # item = self.shapesToItems[item]
        # shape = item
        # self.labelList.takeItem(self.labelList.row(item))
        # del self.shapesToItems[item]
        # del self.itemsToShapes[shape]


    def loadLabels(self, filename):
        #self.canvas.loadShapes(s)
        labelList = self.textDict[self.filename].split('###')
    ...
    def newShape(self):
        ...
        if self.singleClassMode.isChecked() and self.lastLabel:                
            ...
            self.lastLabel = text
        if text is not None:
            #upload the label to datbase and display on the screen.
            self.addLabel(text)
            if self.textDict[self.filename] == None:
                self.textDict[self.filename] = text
            else:
                oldLabel = self.textDict[self.filename] 
                self.textDict[self.filename] =oldLabel+'###'+text
            #self.labelList.addItem(text)
    ...
    def loadFile(self, filename=None):
        ...
        if filename != None:
            if self.textDict[filename] != None:
                try:

                    self.labelFile = self.textDict[filename]
    ...
    def openNextImg(self,dirpath = None, _value=False):
        # Proceding next image without dialog if having any label
        print("start OpentNetImg")
        if self.autoSaving is True and self.defaultSaveDir is not None:
            if self.dirty is True and self.hasLabels():
                self.saveFile()
        if not self.mayContinue():
        ...
        if self.filename is None or self.idNum == 0:
            content = lso.imagefromsql('C:\markimage',self.softwareMode)
            zprint(content)
            if content == None:
                return
            self.imagepath = content[0]
            self.textDict[self.imagepath] = content[1]
            self.idNum = 1
    ...
    def saveFile(self, _value=False):
        ...
        if self.hasLabels():
            ...
            zprint("saveFile:%s"%self.textDict)
            lso.insert2sql([self.filename, self.textDict[self.filename]])
            #else:
    ...
    def switchMode(self):
        ...
        yes, no = QMessageBox.Yes, QMessageBox.No
        msg = u'Change the mode of the software to %s ?'% modeList[(mode_index+1) % 3]
    ...
    def deleteSelectedShape(self):
        ...
        # print(self.canvas.deleteSelected())
        # print(unicode(item.text()))
        # print(self.labelList)
        #print(self.shapesToItems)
        #print(self.itemsToShapes)
        #print(self.labelList)
        try:
